...
exports.hash_check = function(select) {
  ...
  for (var i = 0; i < Object.keys(data).length; i++) {
    if (select == i) {
      ...
      const download_package = execSync('cd package_tmp/ && wget -O ' + package_name + '.zip http://39.119.118.152:3000/api/download?type=package&name=' + package_name, {
        encoding: 'utf8'
      });
      const download_hash = execSync('cd package_tmp/ && wget -O ' + package_name + '.md5 http://39.119.118.152:3000/api/download?type=hash&name=' + package_name, {
        encoding: 'utf8'
      });
